...
ANTS $dim -m  CC[$img1,$img2,1,4]  -t SyN[0.25]  -r Gauss[3,0] -o $out -i 0 --number-of-affine-iterations 1000x1000x500 #fortex
WarpImageMultiTransform $dim $img2  ${out}.nii.gz  ${out}Affine.txt -R $img1 #fortex
...
ImageMath $dim temp.nii.gz TileImages 3 temp.nii.gz ${out}.nii.gz temp2.nii.gz 
ImageMath $dim temp.nii.gz PadImage temp.nii.gz -20
ConvertToJpg temp.nii.gz ../figures/r${num2}registrationaffine.jpg
rm temp*.nii.gz
exit 0 